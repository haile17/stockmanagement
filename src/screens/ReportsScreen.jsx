...
useFocusEffect(
  ...
  // Get current report data based on selected type and filters
  const getCurrentReportData = () => {
    ...
    if (!startDate || !endDate) {
      showWarning('Date Range Required', 'Please select both start and end dates before generating the report');
      return;
    }
    setLoading(true);
    try {
      ...
      showWarning('No Data', 'No data available for the selected report type and date range.');
      return;
    }
      const pdf = await PDFService.generatePDF(reportData, startDate, endDate, selectedReportType);
...
const renderDateFilters = () => {
  return (
    <View style={reportScreenStyles.dateFilterContainer}>
      ...
      {/* Date Picker Modals remain unchanged */}
      <DatePicker
        modal
